...
class ProsumaAPIReceptionExtractor:
    ...
    def setup_logging(self):
        ...
        if log_path:
            logging.basicConfig(
                ...
                handlers=[
                    logging.FileHandler(os.path.join(log_path, f'prosuma_api_reception.log')),
                    SafeStreamHandler()
                ]
            )
        else:
            logging.basicConfig(
                ...
                format='%(asctime)s - %(levelname)s - %(message)s',
                handlers=[
                    logging.FileHandler(f'prosuma_api_reception.log'),
                    SafeStreamHandler()
                ]
            )
    ...
    def export_to_csv(self, data, shop_code, shop_name):
        ...
        # En-têtes CSV
        fieldnames = ['id', 'reference', 'type', 'supplier', 'total_amount', 'status', 'created_at', 'updated_at', 'shop_code', 'shop_name']
