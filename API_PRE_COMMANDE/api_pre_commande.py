#!/usr/bin/env python3
"""
Extracteur API Prosuma RPOS - PRE_COMMANDE
Récupère les données via l'API supplier_pre_order
"""
...
class ProsumaAPIPrecommandeExtractor:
    def __init__(self):
    ...
    def setup_logging(self):
        ...
        if log_path:
            logging.basicConfig(
                ...
                handlers=[
                    logging.FileHandler(os.path.join(log_path, f'prosuma_api_pre_commande.log')),
                    SafeStreamHandler()
                ]
            )
        else:
            logging.basicConfig(
                ...
                format='%(asctime)s - %(levelname)s - %(message)s',
                handlers=[
                    logging.FileHandler(f'prosuma_api_pre_commande.log'),
                    SafeStreamHandler()
                ]
            )
    ...
    def get_network_path_for_shop(self, shop_code):
        ...
        network_path = build_network_path(self.network_folder_base, "PRE_COMMANDE")
    ...
    def export_to_csv(self, data, shop_code, shop_name):
        ...
        filename = f'export_pre_commande_{shop_code}_{timestamp}.csv'
        local_filepath = os.path.join(self.base_dir, filename)
        
        # En-têtes CSV
        fieldnames = ['id', 'reference', 'supplier', 'total_amount', 'status', 'created_at', 'updated_at', 'shop_code', 'shop_name']
    ...
    def extract_shop(self, shop_code):
        ...
        logger.info(f"==================================================")
        logger.info(f"EXTRACTION PRE_COMMANDE MAGASIN {shop_code}")
        logger.info(f"==================================================")
    ...
    def extract_all(self):
        ...
        logger.info("=" * 60)
        logger.info("DÉBUT DE L'EXTRACTION API PROSUMA - PRE_COMMANDE")
        logger.info("=" * 60)
        
                # Créer le dossier réseau au début
        network_path = self.get_network_path_for_shop("PRE_COMMANDE")
        if network_path:
...
def main():
    """Fonction principale"""
    try:
        extractor = ProsumaAPIPrecommandeExtractor()
