#!/usr/bin/env python3
"""
Extracteur API Prosuma RPOS - RETOUR_MARCHANDISE
Récupère les données via l'API delivery_return
"""
...
class ProsumaAPIRetourmarchandiseExtractor:
    def __init__(self):
    ...
    def setup_logging(self):
        ...
        if log_path:
            logging.basicConfig(
                ...
                handlers=[
                    logging.FileHandler(os.path.join(log_path, f'prosuma_api_retour_marchandise.log')),
                    SafeStreamHandler()
                ]
            )
        else:
            logging.basicConfig(
                ...
                format='%(asctime)s - %(levelname)s - %(message)s',
                handlers=[
                    logging.FileHandler(f'prosuma_api_retour_marchandise.log'),
                    SafeStreamHandler()
                ]
            )
    ...
    def get_network_path_for_shop(self, shop_code):
        ...
        network_path = build_network_path(self.network_folder_base, "RETOUR_MARCHANDISE")
    ...
    def export_to_csv(self, data, shop_code, shop_name):
        ...
        filename = f'export_retour_marchandise_{shop_code}_{timestamp}.csv'
        local_filepath = os.path.join(self.base_dir, filename)
        
        # En-têtes CSV
        fieldnames = ['id', 'reference', 'supplier', 'reason', 'total_amount', 'status', 'created_at', 'updated_at', 'shop_code', 'shop_name']
            ...
            with open(local_filepath, 'w', newline='', encoding='utf-8') as csvfile:
                ...
                for item in data:
                    ...
                    for field in fieldnames:
                        if field in ['shop_code', 'shop_name']:
                            if field == 'shop_code':
                                row[field] = shop_code
    ...
    def extract_shop(self, shop_code):
        ...
        logger.info(f"==================================================")
        logger.info(f"EXTRACTION RETOUR_MARCHANDISE MAGASIN {shop_code}")
        logger.info(f"==================================================")
    ...
    def extract_all(self):
        ...
        logger.info("=" * 60)
        logger.info("DÉBUT DE L'EXTRACTION API PROSUMA - RETOUR_MARCHANDISE")
        logger.info("=" * 60)
        
        # Créer le dossier réseau au début
        network_path = self.get_network_path_for_shop("RETOUR_MARCHANDISE")
        if network_path:
...
def main():
    """Fonction principale"""
    try:
        extractor = ProsumaAPIRetourmarchandiseExtractor()
