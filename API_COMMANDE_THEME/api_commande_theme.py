#!/usr/bin/env python3
"""
Extracteur API Prosuma RPOS - Commandes par Thème/Promotion
Récupère les commandes par thème via l'API external_order
"""
...
class ProsumaAPICommandeThemeExtractor:
    def __init__(self):
    ...
    def get_network_path_for_shop(self, shop_code):
        ...
        network_path = build_network_path(self.network_folder_base, "COMMANDE_THEME")
    ...
    def get_external_orders(self, base_url, shop_id, page_size=1000):
    ...
    def export_to_csv(self, orders, shop_code, shop_name):
        ...
        filename = f'export_commande_theme_{shop_code}_{timestamp}.csv'
        local_filepath = os.path.join(self.base_dir, filename)
        
        # En-têtes CSV
        fieldnames = [
            'id', 'reference', 'theme', 'promotion', 'customer_name', 'customer_phone',
            'total_amount', 'status', 'created_at', 'updated_at', 'shop_code', 'shop_name'
        ]
    ...
    def extract_shop(self, shop_code):
        ...
        logger.info(f"==================================================")
        logger.info(f"EXTRACTION COMMANDES THÈME MAGASIN {shop_code}")
        logger.info(f"==================================================")
        ...
        orders = self.get_external_orders(base_url, shop_id)
    ...
    def extract_all(self):
        ...
        logger.info("=" * 60)
        logger.info("DÉBUT DE L'EXTRACTION API PROSUMA - COMMANDES THÈME")
        logger.info("=" * 60)
        
                # Créer le dossier réseau au début
        network_path = self.get_network_path_for_shop("COMMANDE_THEME")
        if network_path:
...
def main():
    """Fonction principale"""
    try:
        extractor = ProsumaAPICommandeThemeExtractor()
